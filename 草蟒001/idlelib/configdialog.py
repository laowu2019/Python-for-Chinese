...
class ConfigDialog(Toplevel):
    ...
    def create_widgets(self):
        ...
        note.add(self.fontpage, text='字体/制表符')
        note.add(self.highpage, text='高亮显示')
        note.add(self.keyspage, text=' 按键 ')
        note.add(self.genpage, text=' 通用 ')
        note.add(self.extpage, text=' 扩展 ')
...
class FontPage(Frame):
    ...
    def create_page_font_tab(self):
        ...
        frame_font = LabelFrame(
                self, borderwidth=2, relief=GROOVE, text=' Shell/编辑器字体 ')
        frame_sample = LabelFrame(
        ...
        font_name_title = Label(
                frame_font_name, justify=LEFT, text='字体名称 :')
        ...
        font_size_title = Label(frame_font_param, text='大小 :')
...
class HighPage(Frame):
    ...
    def create_page_highlight(self):
        ...
        self.button_set_color = Button(
                self.frame_color_set, text='选择颜色用于 :',
...
class KeysPage(Frame):
    ...
    def create_page_keys(self):
        ...
        frame_custom = LabelFrame(
                ...
                text=' 自定义键绑定 ')
        frame_key_sets = LabelFrame(
                self, borderwidth=2, relief=GROOVE, text=' 键组合 ')
        # frame_custom.
        frame_target = Frame(frame_custom)
        target_title = Label(frame_target, text='操作 - 键')
        ...
        self.builtin_keyset_on = Radiobutton(
                ...
                command=self.set_keys_type, text='使用内置键组合')
        self.custom_keyset_on = Radiobutton(
                frames[0], variable=self.keyset_source, value=0,
                command=self.set_keys_type, text='使用自定义键组合')
        self.builtinlist = DynOptionMenu(
    ...
    def delete_custom_keys(self):
        ...
        if not item_list:
            self.custom_keyset_on.state(('disabled',))
            self.customlist.SetMenu(item_list, '- 无自定义键 -')
        else:
            self.customlist.SetMenu(item_list, item_list[0])
...
class GenPage(Frame):
    ...
    def create_page_general(self):
        ...
        startup_title = Label(frame_run, text='启动时')
        ...
        win_size_title = Label(
                frame_win_size, text='初始窗口尺寸 (字符数)')
        win_width_title = Label(frame_win_size, text='宽度')
        self.win_width_int = Entry(
                frame_win_size, textvariable=self.win_width, width=3)
        win_height_title = Label(frame_win_size, text='高度')
        ...
        paren_style_title = Label(frame_paren1, text='括号匹配样式')
        ...
        context_title = Label(frame_context, text='最大上下文行数 :')
        ...
        self.button_helplist_remove = Button(
                frame_helplist_buttons, text='移除', state='disabled',
