...
ast_for_atom_expr(struct compiling *c, const node *n)
{
    ...
    if ((TYPE(CHILD(n, 0)) == NAME && strcmp(STR(CHILD(n, 0)), "await") == 0) || (TYPE(CHILD(n, 0)) == NAME && strcmp(STR(CHILD(n, 0)), "等待") == 0)) {
        start = 1;
        assert(nch > 1);
    }
    ...
    if (s[2] == 's' || (strcmp(s, "否则") == 0)) {
        ...
        if ((TYPE(CHILD(n, (n_elif + 1))) == NAME
            && STR(CHILD(n, (n_elif + 1)))[2] == 's')
             || (TYPE(CHILD(n, (n_elif + 1))) == NAME
              && strcmp(STR(CHILD(n, (n_elif + 1))), "否则") == 0)) {
            has_else = 1;
            n_elif -= 3;
        }
        n_elif /= 4;
...
ast_for_try_stmt(struct compiling *c, const node *n)
{
    ...
    if (TYPE(CHILD(n, nch - 3)) == NAME) {
        if (strcmp(STR(CHILD(n, nch - 3)), "finally") == 0 || strcmp(STR(CHILD(n, nch - 3)), "最后") == 0) {
