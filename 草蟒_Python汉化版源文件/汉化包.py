...
内置函数字典 = {'输出':'print', '输入':'input', '求值':'eval', '绝对值':'abs', '最大值':'max', '最小值':'min', \
    ...
    '迭代对象':'iter', '下一个':'next', '哈希':'hash', '标识':'id', '是否实例':'isinstance', '是否子类':'issubclass', \
    ...
    '局部符号字典':'locals', '全局符号字典':'globals', '可显字符':'repr', '迭代器':'zip', '__导入__':'__import__', \
    '退出':'exit'}  # ascii 中英文相同

其他名称字典 = {'自己':'self', '本类':'cls', '__初始化__':'__init__'}

常量字典 = {"'红'":"'red'", "'黄'":"'yellow'", "'蓝'":"'blue'", "'黑'":"'black'", "'白'":"'white'", \
    "'绿'":"'green'", "'左'":"'left'", "'右'":"'right'", "'居中'":"'center'", "'正常'":"'normal'", \
    "'斜体'":"'italic'", "'粗体'":"'bold'", "'方形'":"'square'", "'点'":"'dot'"}
